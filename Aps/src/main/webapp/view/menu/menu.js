...
function divConstructor(dados) {
    ...
    botaoLanche1.onclick = () => {
        showIngredientes(dados['id_lanche']);
        campoSelecionado = p;
    };
    ...
    botaoLanche2.onclick = () => {
        lancheProCarrinho(dados['nome'], dados['valor_venda']);
    };
...
function divConstructorBebidas(dados) {
    ...
    botaoLanche2.innerText = "Adicionar ao Carrinho"
    botaoLanche2.onclick = () => {
        bebidaProCarrinho(dados['nome'], dados['valor_venda']);
    };
...
function getLanches(resposta) {
    console.log("entoru no get lanches")
    if (!resposta) {
        alert("sem resposta!")
    }
